...
export default async function middleware(req) {
  ...
  try {
    const res = await fetch(`https://api.campytech.com/blog/slug/${slug}`);
    const post = await res.json();

    const firstBlock = post.content?.blocks?.find(b =>
      b.type === 'paragraph' &&
      b.data?.text &&
      !b.data.text.startsWith('#') &&
      b.data.text.length > 30
    );
    const description = firstBlock?.data?.text?.replace(/<[^>]*>/g, '').slice(0, 160) || '';
